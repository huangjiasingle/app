...
import (
	"app/pkg/resouces/deployment"
	"app/pkg/resouces/service"
	"context"
	"encoding/json"
	"reflect"
...
func (r *ReconcileApp) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.client.Get(context.TODO(), request.NamespacedName, deploy); err != nil && errors.IsNotFound(err) {
		// 创建deploy
		if err := r.client.Create(context.TODO(), deployment.New(instance)); err != nil {
			return reconcile.Result{}, err
		}
		// 创建svc
		if err := r.client.Create(context.TODO(), service.New(instance)); err != nil {
			return reconcile.Result{}, err
		}
